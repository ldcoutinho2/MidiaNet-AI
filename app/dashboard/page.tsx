...
type Entitlement = {active:boolean;trialActive:boolean;paidActive:boolean;contentRemaining:number;contentLimit:number;imageRemaining:number;imageLimit:number;contentUsed:number;imageUsed:number;};
...
function isCurrentStrategy(v:any):v is AIProfile{
 return Boolean(v&&typeof v==="object"&&typeof v.objective==="string"&&Array.isArray(v.weeklyPlan)&&v.weeklyPlan.length>=2&&v.weeklyPlan.length<=7&&v.weeklyPlan.every((d:any)=>d&&Array.isArray(d.slots)&&d.slots.length>=3&&d.slots.length<=4));
}
function SetupStrategy({analyzing,analyze,hasOldStrategy}:{analyzing:boolean;analyze:()=>void;hasOldStrategy:boolean}){return <div className="feature" style={{marginTop:24}}><div className="badge">{hasOldStrategy?"Atualização":"1º passo"}</div><h2 style={{marginTop:12}}>{hasOldStrategy?"Sua estratégia ganhou a nova programação.":"Vamos transformar seu perfil em um plano."}</h2><p className="muted" style={{marginTop:8,maxWidth:760}}>A IA vai montar diagnóstico, estratégia, semana completa e conteúdos prontos para você executar — 3 conteúdos por dia, com um 4º opcional quando fizer sentido.</p><button className="btn primary" onClick={analyze} disabled={analyzing} style={{marginTop:18}}>{analyzing?"Analisando seu perfil...":hasOldStrategy?"Atualizar minha estratégia →":"Montar minha estratégia →"}</button></div>}
...
function Audit({audit,onAnalyze,analyzing}:{audit?:ProfileAudit;onAnalyze:()=>void;analyzing:boolean}){
 const safeAudit:ProfileAudit|null = audit ? {
   ...
   bio: {...(audit.bio||{}),score:Number(audit.bio?.score ?? 0),diagnosis:audit.bio?.diagnosis || "Não avaliado.",impact:audit.bio?.impact || "",recommendation:audit.bio?.recommendation || "Faça uma revisão da bio.",suggestedBio:audit.bio?.suggestedBio || ""},
   profilePhoto: {...(audit.profilePhoto||{}),score:Number(audit.profilePhoto?.score ?? 0),diagnosis:audit.profilePhoto?.diagnosis || "Não avaliado.",recommendation:audit.profilePhoto?.recommendation || "Revise a foto de perfil."},
   nameAndPositioning: {...(audit.nameAndPositioning||{}),score:Number(audit.nameAndPositioning?.score ?? 0),diagnosis:audit.nameAndPositioning?.diagnosis || "Não avaliado.",recommendation:audit.nameAndPositioning?.recommendation || "Deixe nome e posicionamento claros."},
   highlights: {...(audit.highlights||{}),score:Number(audit.highlights?.score ?? 0),diagnosis:audit.highlights?.diagnosis || "Não avaliado.",recommendation:audit.highlights?.recommendation || "Organize os destaques principais."},
   grid: {...(audit.grid||{}),score:Number(audit.grid?.score ?? 0),diagnosis:audit.grid?.diagnosis || "Não avaliado.",recommendation:audit.grid?.recommendation || "Crie uma identidade visual coerente."},
   postingFrequency: {...(audit.postingFrequency||{}),score:Number(audit.postingFrequency?.score ?? 0),diagnosis:audit.postingFrequency?.diagnosis || "Não avaliado.",recommendation:audit.postingFrequency?.recommendation || "Mantenha uma rotina consistente."},
   conversion: {...(audit.conversion||{}),score:Number(audit.conversion?.score ?? 0),diagnosis:audit.conversion?.diagnosis || "Não avaliado.",recommendation:audit.conversion?.recommendation || "Defina uma CTA clara.",ctaSuggestion:audit.conversion?.ctaSuggestion || ""},
 ...
 async function shareAudit(){
   ...
   ctx.fillStyle="#ffffff";ctx.font="900 170px Arial";ctx.fillText(String(safeAudit.overallScore),70,370);
   ...
   const items:[string,number][]=[["Nome e @",safeAudit.nameAndPositioning.score],["Foto de perfil",safeAudit.profilePhoto.score],["Bio",safeAudit.bio.score],["Destaques",safeAudit.highlights.score],["Grade / feed",safeAudit.grid.score],["Frequência",safeAudit.postingFrequency.score],["Conversão",safeAudit.conversion.score]];
   ...
   ctx.fillStyle="#d4d4d8";ctx.font="28px Arial";const lines=(safeAudit.summary||"").match(/.{1,55}(?:\s|$)/g)||[];y+=35;for(const line of lines.slice(0,6)){ctx.fillText(line.trim(),70,y);y+=42}
 ...
 const items=[
   ...
   ["Bio",safeAudit.bio],
   ["Destaques",safeAudit.highlights],
   ["Grade / feed",safeAudit.grid],
   ["Frequência",safeAudit.postingFrequency],
   ["Conversão",safeAudit.conversion]
 ] as const;
...
function Results({profile,onNewIdea}:{profile:Profile;onNewIdea:()=>void}){
 ...
 return <div style={{marginTop:8}}>
  ...
  {nextTest&&<div className="feature" style={{marginTop:14}}><div className="badge">🧠 PRÓXIMO TESTE</div><h2 style={{marginTop:10}}>{nextTest.title}</h2><p className="muted" style={{marginTop:7}}>{nextTest.reason}</p><div className="card" style={{marginTop:12}}><strong>Formato sugerido: {nextTest.format}</strong><p className="small muted" style={{marginTop:6}}>Não copie o conteúdo. Repita a lógica que funcionou e teste uma nova execução.</p></div><button className="btn primary" style={{marginTop:12}} onClick={onNewIdea}>＋ Criar variação agora</button></div>}
