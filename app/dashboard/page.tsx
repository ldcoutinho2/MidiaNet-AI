...
const tabs=[["overview","🏠 Visão geral"],["dna","🧬 Meu DNA"],["ideas","💡 Ideias"],["scripts","🎬 Roteiros"],["captions","✍️ Legendas"],["planner","📅 Planejamento"],["results","📊 Resultados"],["profile","👤 Meu perfil"]];
...
export default function Dashboard(){
 ...
 async function analyze(){setAnalyzing(true);setError("");try{const r=await fetch("/api/ai/analyze-profile",{method:"POST"});const x=await r.json();if(!r.ok){setError(x.error||"Não foi possível analisar.");return}setData(d=>d?({...d,user:{...d.user,strategicProfile:{...d.user.strategicProfile,aiProfile:x.aiProfile,aiAnalyzedAt:x.aiAnalyzedAt}}}):d);setTab("dna")}catch{setError("Não foi possível conectar à IA.")}finally{setAnalyzing(false)}}
